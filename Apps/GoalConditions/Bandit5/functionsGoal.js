...
Shiny.addCustomMessageHandler("envHandler",
	function(message){
...
function add(a, b) {
	/* I don't know why exactly but this function later
...
function updateValue(elmtG, elmtGO, elmtP, elmtC, distd, dist, distdd, distddd, ind, sel, outcome, outcomeCum, selection, nTrials, gameNr, respTime, trial, t,
	...
	if (ind.length < nTrials){
		...
		if (clickEnabled[clickEnabled.length - 1] === 1){
			...
			// set the color of the point counter value (red if under goal, green if above)
			if (outcome.reduce(add, 0) < goal){
				pointsP.style.color = "#FF6A6A";
			} else {
				pointsP.style.color = "#00CD00";
			}
