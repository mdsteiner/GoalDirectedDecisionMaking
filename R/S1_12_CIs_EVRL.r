...
sink("C:/Users/Markus/Dropbox/Masterarbeit/eegoals/documents/CIsEVRL.txt")
...
m.chv <- lme4::glmer(high.var.chosen ~ choose.highvar.subj.ev.rl + (1|game) + (1|id.f),
...
m.chv <- lme4::glmer(high.var.chosen ~ choose.highvar.subj.ev.rl + (1|id.f/game),
                     data = subset(df.trial, goal.condition == "Goal" & game > 1), family = binomial)
summary(m.chv)
print("Tjurs D:")
r2(m.chv)
...
m.pa <- lme4::glmer(pred.RSF.acc.ev.rl ~ goal.condition.f + (1|game) + (1|id.f),
                    data = subset(df.trial, game > 1 & pred.EV.ev.rl != pred.RSF.ev.rl), family = binomial)
...
m.pa <- lme4::glmer(pred.RSF.acc.ev.rl ~ goal.condition.f + (1|id.f/game),
                    data = subset(df.trial, game > 1 & pred.EV.ev.rl != pred.RSF.ev.rl), family = binomial)
