...
sink("C:/Users/Markus/Dropbox/Masterarbeit/eegoals/documents/CIsPTRL.txt")
...
m.chv <- lme4::glmer(high.var.chosen ~ choose.highvar.subj.pt.rl + (1|game) + (1|id.f),
...
m.chv <- lme4::glmer(high.var.chosen ~ choose.highvar.subj.pt.rl + (1|id.f/game),
                     data = subset(df.trial, goal.condition == "Goal" & game > 1), family = binomial)
summary(m.chv)
print("Tjurs D:")
r2(m.chv)
...
m.pa <- lme4::glmer(pred.RSF.acc.pt.rl ~ goal.condition.f + (1|game) + (1|id.f),
                    data = subset(df.trial, game > 1 & pred.EV.pt.rl != pred.RSF.pt.rl), family = binomial)
...
m.pa <- lme4::glmer(pred.RSF.acc.pt.rl ~ goal.condition.f + (1|id.f/game),
                    data = subset(df.trial, game > 1 & pred.EV.pt.rl != pred.RSF.pt.rl), family = binomial)
