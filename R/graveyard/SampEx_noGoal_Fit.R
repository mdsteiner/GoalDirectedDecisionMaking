...
for (ii in seq_len(N_sub)){
  ...
  grid_SampEx <- apply(startp_SampEx, 1, SampEx_noGoalFit, chd = dat$selection[fit_ind],
  ...
  r_good_SampEx[ii] <- SampEx_noGoalFit(startp_SampEx[which.min(grid_SampEx),],
  ...
  r_pred_SampEx[ii] <- mean(SampEx_noGoalFit(startp_SampEx[which.min(grid_SampEx),],
                                      chd = dat$selection[fit_ind],
                                      v_As = v_As_temp, v_Bs = v_Bs_temp,
                                      T = N_trial, curr_t = dat$trial[fit_ind],
                                      goal = Goal, curr_Y = dat$points.cum[fit_ind],
...
save.image("data/Study1Data/useData/SampExHeur_noGoal_Fits.RData")
...
# 
# 
#     tfit_SampEx <- optim(startp_SampEx[which.min(grid_SampEx),], SampEx_noGoalFit,
...
# 
#   r_pred_SampEx[ii] <- mean(SampEx_noGoalFit(fit_SampEx$par,chd = dat$selection[fit_ind],
#                                       v_As = v_As_temp, v_Bs = v_Bs_temp,
#                                       T = N_trial, curr_t = dat$trial[fit_ind],
#                                       goal = Goal, curr_Y = dat$points.cum[fit_ind],
#                                       pred=TRUE))
