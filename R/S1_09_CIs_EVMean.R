...
sink("C:/Users/Markus/Dropbox/Masterarbeit/eegoals/documents/CIsEV.txt")
...
m.chv <- lme4::glmer(high.var.chosen ~ choose.highvar.subj + (1|id.f/game),
                     data = subset(df.trial, goal.condition == "Goal" & game > 1), family = binomial)
summary(m.chv)
print("Tjurs D:")
r2(m.chv)
...
m.pa <- lme4::glmer(pred.RSF.acc ~ goal.condition.f + (1|id.f/game),
                    data = subset(df.trial, game > 1 & pred.EV != pred.RSF), family = binomial)
